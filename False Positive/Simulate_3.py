...
def read(name):
    ...
    with open(name, 'rb') as f:
        for line in f:
            ...
            if len(a_line) == 4:
                if (a_line[3] != "Second(s)\r\n"):
                    frequency.append(float(a_line[3]))
    f.close()
    for i in range(0, len(frequency)):
        numbers[i] = numbers[i] + (frequency[i],)
    return numbers
                        ...
                        if rgeno[0] in observed and rgeno[1] not in observed:
                            for j in range(0, len(master[case][race][locus])):
                                if master[case][race][locus][j][1][0] == rgeno[0] and master[case][race][locus][j][1][1] == float(-1):
                                    ratio = uniform(0, 1)
                                    if ratio >= master[case][race][locus][j][2]:
                                        boolean = False
                                    else:
                                        r = randint(0, len(GENO[locus]) - 1)
                                        rgeno = GENO[locus][r]
                            ...
                            ratio = uniform(0, 1)
                            if ratio >= master[case][race][locus][len(master[case][race][locus]) - 1][2]:
                                boolean = False
                            else:
                                r = randint(0, len(GENO[locus]) - 1)
                                rgeno = GENO[locus][r]
